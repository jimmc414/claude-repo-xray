...
- **Foundation** (92 modules): Core utilities, configuration, data models
- **Core** (297 modules): Business logic and domain services
...
|--------------|------|-------------|
| `main` | `main.py` | Entry point (main.py) |
| `cli` | `cli.py` | Entry point (cli.py) |
| `__main__` | `__main__.py` | Entry point (__main__.py) |
| `interactive` | `interactive.py` | Has __main__ block |
| `cache` | `cache.py` | Has __main__ block |
| `config` | `config.py` | Has __main__ block |
| `graph` | `graph.py` | Has __main__ block |
| `history` | `history.py` | Has __main__ block |
| `run` | `run.py` | Has __main__ block |
| `status` | `status.py` | Has __main__ block |
...
### Tested Modules
`agents`, `analysis`, `biology`, `cli`, `compression`, `core`, `data_analysis`, `db`, `domains`, `execution`, `experiments`, `hypothesis`, `integration`, `knowledge`, `literature`

### Potentially Untested
*Source directories without corresponding test directories:*

`Figure_3_perovskite_solar_cell/`, `api/`, `claude/`, `config/`, `kosmos-agentic-data-scientist/`, `kosmos-claude-scientific-writer/`, `kosmos-claude-skills-mcp/`, `kosmos-karpathy/`

### Key Fixtures (conftest.py)
*Available test fixtures for setup:*

`alert_tracker`, `arxiv_response_xml`, `circuit_breaker`, `cleanup_test_files`, `cli_runner`, `configured_metrics_collector`, `convergence_detector`, `deepseek_client`, `e2e_artifacts_dir`, `event_loop`, `fixtures_dir`, `in_memory_world_model`
