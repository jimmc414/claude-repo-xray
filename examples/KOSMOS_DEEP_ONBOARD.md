...
`run_research()` (kosmos/cli/commands/run.py:51)
  ...
  -> `get_config()` (kosmos/config.py:1140) -- Singleton pattern. Creates `KosmosConfig()` from environment, calls `create_directories()`. [FACT] (config.py:1151-1153) (see Shared State: Config Singleton)

  -> `ResearchDirectorAgent.__init__()` (kosmos/agents/research_director.py:68) (see Module Index: kosmos/agents/research_director.py) -- Initializes: research plan (Pydantic `ResearchPlan`), workflow state machine (`ResearchWorkflow`), LLM client via `get_client()`, database via `init_from_config()`, convergence detector, world model (Neo4j knowledge graph), rollout tracker, error recovery state, async locks. [FACT] (research_director.py:68-260)

    Sub-hops during __init__:
    -> `get_client()` (kosmos/core/llm.py:613) -- Singleton. Tries provider system from config, falls back to `AnthropicProvider`. [FACT] (llm.py:652-664) (see Module Index: kosmos/core/llm.py) (see Shared State: LLM Client Singleton)
    -> `init_from_config()` (kosmos/db/__init__.py:140) -- Runs `first_time_setup()`, creates SQLAlchemy tables. [SIDE EFFECT: table creation] (db/__init__.py:140-179) (see Error Handling: Database Error Handling)
    -> `get_world_model()` (kosmos/world_model/factory.py:55) -- Singleton. Returns `Neo4jWorldModel` wrapping `KnowledgeGraph`. [FACT] (factory.py:55-104) (see Module Index: kosmos/world_model/) (see Shared State: World Model Singleton)
  ...
  -> `get_registry()` (kosmos/agents/registry.py:512) -- Singleton registry (see Hazards: kosmos/agents/registry.py)
  -> `registry.register(director)` (registry.py:70) -- Adds to _agents dict, sets message router callback (see Gotcha #11)
  ...
  -> `decide_next_action()` (research_director.py:2388) -- State Machine Decision (see Module Index: kosmos/core/workflow.py)
    [FACT] Decision tree based on `workflow.current_state`. Contains budget check, runtime check, loop-guard (MAX_ACTIONS_PER_ITERATION=50). (research_director.py:2388-2548) (see Gotcha #38, #39)
  ...
  -> `_handle_generate_hypothesis_action()` (research_director.py:1391)
    -> `HypothesisGeneratorAgent(config=self.config)` -- lazy-init (research_director.py:1404) (see Critical Path 4a for full trace)
    -> `agent.generate_hypotheses(question, num=3, domain, store_in_db=True)` (research_director.py:1408)
      ...
      -> `NoveltyChecker.check_novelty(hyp)` (hypothesis_generator.py:213) -- novelty scoring (see Gotcha #61, #62)
  ...
  -> `_handle_design_experiment_action(hypothesis_id)` (research_director.py:1458) (see Critical Path 5e for full trace)
    -> `ExperimentDesignerAgent(config=self.config)` -- lazy-init (research_director.py:1470)
    -> `agent.design_experiment(hypothesis_id=hyp_id, store_in_db=True)` (research_director.py:1474) (see Module Index: kosmos/models/experiment.py)
  ...
  -> `_handle_execute_experiment_action(protocol_id)` (research_director.py:1521) (see Critical Path 5 for full execution pipeline)
    -> `ExperimentCodeGenerator(use_templates=True, use_llm=True)` -- lazy-init (research_director.py:1538) (see Module Index: kosmos/execution/code_generator.py)
    -> `CodeExecutor(max_retries=3)` -- lazy-init (research_director.py:1540) (see Module Index: kosmos/execution/executor.py)
    ...
    Code execution -- CRITICAL TERMINAL SIDE EFFECT (see Gotcha #6, #7, #8 for security considerations):
    -> `code_executor.execute(code, retry_on_error=True)` (research_director.py:1572)
      ...
      -> `execute()` (executor.py:237)
        -> `_execute_once(code, local_vars)` (executor.py:282)
          -> if use_sandbox: `_execute_in_sandbox(code, local_vars)` (executor.py:474-475) -- Docker sandbox (see Critical Path 5c)
          -> else: `_exec_with_timeout(code, exec_globals, exec_locals)` (executor.py:506) (see Gotcha #173 for silent Docker fallback)
  ...
  -> `_handle_analyze_result_action(result_id)` (research_director.py:1666) (see Critical Path 5d for full trace)
  ...
  -> `_handle_refine_hypothesis_action(hypothesis_id)` (research_director.py:1796) (see Module Index: kosmos/models/hypothesis.py)
  ...
  -> `_handle_convergence_action()` (research_director.py:1334) (see Module Index: kosmos/core/convergence.py)
    -> `_apply_multiple_comparison_correction()` (research_director.py:1343) -- BH-FDR correction
    -> `_check_convergence_direct()` (research_director.py:1347) (see Gotcha #15 for empty results list)
  ...
  -> `workflow.transition_to(GENERATING_HYPOTHESES)` (research_director.py:2716-2728) -- resets to safe state (see Error Handling: Research Director Error Recovery)
    [FACT] `MAX_CONSECUTIVE_ERRORS = 3` with exponential backoff `[2, 4, 8]` seconds. (research_director.py:44-46) (see Gotcha #38 for hasattr-based counter initialization)
...
This is the 7-phase evaluation harness that validates the research pipeline end-to-end. It creates `ResearchDirectorAgent` instances (see Module Index: kosmos/agents/research_director.py), runs them through abbreviated research loops, and scores the results. (see Gotcha #1 for destructive DB reset per phase, Gotcha #17 for no aggregate pass/fail)
...
`async def main(output_dir, research_question, domain, data_path, max_iterations)` (scientific_evaluation.py:1342)
  ...
  -> `run_phase2_smoke_test()` (scientific_evaluation.py:1385)
    -> `_reset_eval_state()` (scientific_evaluation.py:262) -- isolation between phases (see Gotcha #1 for data loss risk)
      -> `reset_database()` (kosmos/db/__init__.py:191) -- [SIDE EFFECT: drops and recreates all DB tables] (see Error Handling: Database Error Handling)
...
#### Sub-Path 3a: `run_persona_eval.py:main()` -- Persona Evaluation Flow (see Gotcha #2 for destructive state deletion)
...
`main()` (run_persona_eval.py:258)
  ...
  -> `run_tier1(persona_name, persona, run_dir, dry_run)` (run_persona_eval.py:148)
    -> [SIDE EFFECT: file.unlink] deletes `PROJECT_ROOT/kosmos.db` for clean eval (run_persona_eval.py:187) (see Gotcha #2)
    -> [SIDE EFFECT: shutil.rmtree] deletes `PROJECT_ROOT/.kosmos_cache` (run_persona_eval.py:190) (see Gotcha #2)
    -> [SUBPROCESS] `python scientific_evaluation.py --output-dir {run_dir}/tier1 --research-question {question} --domain {domain} --data-path {dataset} --max-iterations {max_iterations}` (run_persona_eval.py:196)
      -> `scientific_evaluation.main()` (scientific_evaluation.py:1342) -- Full 7-phase pipeline as described in Critical Path 2 above, including:
...
#### Sub-Path 3b: `compare_runs.py:main()` -- Run Comparison Flow (see Hazards: compare_runs.py)
...
`HypothesisGeneratorAgent.execute(message: AgentMessage)` (kosmos/agents/hypothesis_generator.py:91) (see Module Index: kosmos/agents/base.py for BaseAgent lifecycle) (see Gotcha #65 for signature incompatibility)
  ...
  -> `generate_hypotheses(research_question, num_hypotheses, domain, store_in_db)` (hypothesis_generator.py:142)
    ...
    Step 1: `_detect_domain(research_question)` (hypothesis_generator.py:259) -- LLM call (see Module Index: kosmos/core/llm.py for ClaudeClient)
      Calls `self.llm_client.generate()` with `max_tokens=50, temperature=0.0`. Normalizes response to lowercase+underscores. Falls back to `"general"` on any exception (line 286). [FACT: hypothesis_generator.py:286] (see Error Handling: Catch-Log-Degrade)
      [SIDE EFFECT: LLM API call (Anthropic messages.create)]

    Step 2: `_gather_literature_context(research_question, domain)` (hypothesis_generator.py:289) -- Literature search (see Module Index: kosmos/literature/base_client.py for PaperMetadata type)
      Delegates to `self.literature_search.search(query, max_results=self.max_papers_context)`. Returns empty list on error (line 320). Max papers default: 10 (line 82). (see Gotcha #63 for truncation to top 5 papers)

      -> `UnifiedLiteratureSearch.search(query, max_results_per_source, ...)` (kosmos/literature/unified_search.py:76) (see Gotcha #66 for partial timeout results)
    ...
    Step 5: `NoveltyChecker.check_novelty(hypothesis)` (kosmos/hypothesis/novelty_checker.py:72) -- Novelty scoring (see Module Index: kosmos/knowledge/vector_db.py for vector search)
      Called from hypothesis_generator.py:216. Instantiated with `similarity_threshold = 1.0 - self.min_novelty_score` [FACT: hypothesis_generator.py:211]. (see Gotcha #64 for threshold inversion)
      Sub-steps:
    ...
    Step 6: `_store_hypothesis(hypothesis)` (hypothesis_generator.py:463) -- DB write (terminal side effect) (see Error Handling: Database Error Handling)
      1. Opens `get_session()` context manager (see Gotcha #36 for auto-commit behavior)
...
#### Sub-Path 4b: LiteratureAnalyzerAgent.execute() Full Chain (see Gotcha #65 for signature differences, Gotcha #42 for Neo4j silent degradation)
...
#### Sub-Path 5a: ExperimentCodeGenerator.generate() -- Template Matching + LLM Fallback (see Module Index: kosmos/execution/code_generator.py)

`ExperimentCodeGenerator.generate()` (kosmos/execution/code_generator.py:797) (see Gotcha #56, #57 for seed and template-order gotchas)
...
#### Sub-Path 5b: CodeExecutor.execute() -- Sandbox Execution with Retry (see Module Index: kosmos/execution/executor.py) (see Error Handling: Code Execution Retry)
...
#### Sub-Path 5c: DockerSandbox.execute() -- Container Isolation (see Gotcha #5 for return value loss)
...
#### Sub-Path 5d: DataAnalystAgent.interpret_results() -- LLM-Powered Result Analysis (see Module Index: kosmos/models/result.py for ExperimentResult model)
...
#### Sub-Path 5e: ExperimentDesignerAgent.execute() -- Protocol Design (see Module Index: kosmos/models/experiment.py for ExperimentProtocol model) (see Gotcha #19 for LLM enhancement no-op, Gotcha #48 for empty fallback defaults)
...
#### Sub-Path 5f: Research Director Orchestration of Execution Pipeline (see Module Index: kosmos/agents/research_director.py)

`_handle_execute_experiment_action(protocol_id)` (research_director.py:1521) (see Gotcha #40 for _json_safe nested function, Gotcha #35 for placeholder timestamps)
...
This module defines the `BaseAgent` class from which all Kosmos agents (ResearchDirector, HypothesisGenerator, ExperimentDesigner, DataAnalyst, LiteratureAnalyzer, StageOrchestrator) inherit, providing lifecycle management, async inter-agent message passing via `AgentMessage` objects, state persistence, and health monitoring. [FACT: base.py:97-111] (see Critical Path 1 for ResearchDirectorAgent lifecycle, Critical Path 4a for HypothesisGeneratorAgent, Critical Path 4b for LiteratureAnalyzerAgent)
...
Implements the Anthropic (Claude) LLM provider with sync/async generation, streaming, structured JSON output, response caching, auto model selection (Haiku/Sonnet), and cost tracking -- supporting both real API keys and a "CLI mode" where an all-9s key routes requests through Claude Code. [FACT: anthropic.py:1-5] (see Critical Path 1 and 4a for how this provider is invoked via ClaudeClient/get_client()) (see Gotcha #24-#27 for usage stats and cost tracking issues)
...
Defines the abstract base class (`BaseLiteratureClient`) and unified data types (`PaperMetadata`, `Author`, `PaperSource`) for all literature API integrations. Every literature client (arXiv, Semantic Scholar, PubMed) inherits from `BaseLiteratureClient` and returns `PaperMetadata` objects. This is the type contract layer for the entire literature subsystem. [FACT] (see Critical Path 4a Step 2 and Critical Path 4b for usage in hypothesis generation and literature analysis)
...
Generates executable Python code from `ExperimentProtocol` objects (see Module Index: kosmos/models/experiment.py) using a hybrid approach: first attempting template matching against 5 built-in experiment type templates (t-test, correlation, log-log scaling, ML, generic computational), falling back to LLM-based generation via Claude, with a minimal hardcoded template as the last resort. [FACT: code_generator.py:1-9, 786-793] (see Critical Path 5a for full generation trace)
...
Executes generated Python (and optionally R) code with safety sandboxing, output capture, timeout enforcement, optional profiling, determinism testing, and self-correcting retry logic that can modify failing code using pattern-based fixes or LLM-assisted repair. [FACT: executor.py:1-6] (see Critical Path 5b for full execution trace, Critical Path 5c for Docker sandbox) (see Error Handling: Code Execution Retry for retry strategy details)
...
Defines the Pydantic data models for the hypothesis lifecycle: generation requests, hypothesis objects with scores and evolution tracking, generation responses, novelty reports, testability reports, and prioritized hypothesis wrappers. This is a pure data-model file -- it contains no business logic beyond validation and serialization. [FACT] (see Critical Path 4a for hypothesis generation pipeline) (see Critical Path 1 GENERATE_HYPOTHESIS and REFINE_HYPOTHESIS handlers)
...
Provides a unified LLM client interface supporting Anthropic (API and CLI mode) and OpenAI-compatible providers through a singleton pattern; all LLM access in the codebase funnels through `get_client()` or `get_provider()`. [FACT: llm.py:1-12] (see Critical Path 1 for how the research director uses get_client(), see Critical Path 4a for structured generation in hypothesis pipeline) (see Shared State: LLM Client Singleton) (see Gotcha #27, #30 for cost hardcoding and dual client systems)
...
Provides a structured logging system with JSON and text output modes, a module-level `ContextVar` for cross-async correlation IDs, and an `ExperimentLogger` class that tracks experiment lifecycle events with timing and structured metadata. It wraps Python's stdlib `logging` with Kosmos-specific formatters and configuration. [FACT] (see Gotcha #82 for unbounded events list, Error Handling: Logging Infrastructure)
...
Defines the abstract `LLMProvider` interface and supporting data types (`Message`, `UsageStats`, `LLMResponse`, `ProviderAPIError`) that all LLM provider implementations (Anthropic, OpenAI, LiteLLM) must implement, enabling Kosmos to swap between LLM backends via a unified API for text generation, structured output, streaming, and usage tracking. [FACT: base.py:1-6] (see Error Handling: Custom Exception Hierarchy for ProviderAPIError recoverability) (see Gotcha #29, #174 for usage stats skip and is_recoverable order dependence)
...
The master orchestrator for autonomous research: it drives the full research cycle (hypothesize -> design experiment -> execute -> analyze -> refine -> iterate) by coordinating 6+ specialized agents and utility classes through a workflow state machine, managing convergence detection, error recovery, and knowledge graph persistence. [FACT: research_director.py:1-12] (see Critical Path 1 for the complete `kosmos run` trace through this module, Critical Path 2 for evaluation usage) (see Error Handling: Agents Layer for error recovery strategy) (see Gotcha #11-#16 for dead code and misleading counters)
...
Defines the Pydantic data models for experiment results -- `ExperimentResult`, `StatisticalTestResult`, `VariableResult`, `ExecutionMetadata`, `ResultStatus`, and `ResultExport` -- serving as the canonical schema for all experiment output flowing through the system. [FACT: result.py:1-6] (see Critical Path 1 EXECUTE_EXPERIMENT and ANALYZE_RESULT handlers for how results are created and consumed) (see Critical Path 5d for result interpretation)
...
### kosmos/orchestration/ (stage_orchestrator and 4 orchestration modules) (see Gotcha #28 for provider bypass, Gotcha #170 for mock fallback)
...
Wraps ChromaDB to provide persistent vector storage and cosine-similarity search for scientific papers, handling embedding computation (via SPECTER), batched insertion, metadata extraction, and a singleton access pattern for the database instance. [FACT: vector_db.py:1-5] (see Critical Path 4a Step 5 for how NoveltyChecker uses vector search) (see Critical Path 4b find_related branch for semantic paper discovery)
...
Defines the research workflow state machine -- the finite set of states (`WorkflowState`), allowed transitions between them (`ResearchWorkflow`), and the research plan data structure (`ResearchPlan`) that tracks hypotheses, experiments, and results across iterations. [FACT: workflow.py:1-7] (see Critical Path 1 for how the research director drives state transitions) (see Gotcha #75-#79 for workflow state machine quirks)
...
### kosmos/world_model/ (5 modules) (see Critical Path 1 for how the research director persists to world model, see Gotcha #41, #43 for silent degradation)
...
### Hidden Coupling: The Flat Config Dict Anti-Pattern (see Critical Path 1 for how flat_config is created at run.py:148-171)
...
### Hidden Coupling: String-Based Agent Type Matching (see Gotcha #11-#14 for dead message routing)
...
| `core/providers/anthropic.py` | Wraps all SDK errors into ProviderAPIError (see Gotcha #174) | Loses original exception type |
| `research_director.py` | 27 except-Exception sites (highest count) | Must-never-crash orchestrator |
| `core/metrics.py` | BudgetExceededError raised but never caught (see Error Handling section 10) | Unhandled exception crashes run |
...
| `code_generator.py:_validate_syntax` | Raises ValueError, not SyntaxError (see Module Index: kosmos/execution/code_generator.py) | Changes exception type for callers |
```
...
| `_knowledge_graph` (Neo4j) | `knowledge/graph.py:1000` | `get_knowledge_graph()` | May auto-start Docker (120s blocking); falls back to disconnected (see Gotcha #83, #172) |
| `_world_model` (World model) | `world_model/factory.py:52` | `get_world_model()` | **NOT thread-safe** (documented); falls back to InMemoryWorldModel (see Gotcha #43) |
...
| `ResearchPlan` (in-memory) | `core/workflow.py:57` | `ResearchDirectorAgent` only | Protected by async + threading locks; state can diverge from workflow (see Gotcha #79) |
...
### 7. BaseAgent Subclassing Contract (see Module Index: kosmos/agents/base.py for full public API)
...
### 21. Dual Model Pattern (Pydantic + SQLAlchemy) (see Module Index: kosmos/models/result.py for the Pydantic side, see Error Handling: Database Error Handling for the SQLAlchemy side)
...
### 23. Error Handling: Fail-Open / Degrade Gracefully (see Error Handling: Dominant Strategy for full codebase analysis)
...
### 25. Provider Interface Compliance (see Module Index: kosmos/core/providers/base.py for the LLMProvider ABC)
...
5. **Sandbox return value loss** — DockerSandbox extracts return values via stdout "RESULT:" prefix, but generated code stores results in local variable `results`. Non-sandbox path works correctly via `exec_locals.get('results')`. Sandbox may silently lose result data. [FACT] (`sandbox.py:442` vs `code_generator.py:187`) (see Critical Path 5c for Docker sandbox execution trace)
...
11. **Message routing is dead code at runtime** — The `AgentRegistry.register(director)` at run.py:182 sets up message routing, but all agents use direct calls (Issue #76 fix). The registry registration is cosmetic. [FACT] (`research_director.py:1391-1397 comments`, `registry.py:70-97`) (see Critical Path 1 for the active direct-call pattern, see Hazards: kosmos/agents/registry.py)
...
19. **LLM enhancement is a no-op** — `_enhance_protocol_with_llm()` calls the LLM but never parses or applies the response to the protocol. Logs "applied" but returns protocol unchanged. Wastes API cost. [FACT] (`experiment_designer.py:714-717`) (see Critical Path 5e Step 4 for where this is called, see Hazards: experiment_designer.py:714-717)
...
28. **PlanCreator and PlanReviewer bypass the provider layer entirely** — They call `anthropic_client.messages.create()` directly, meaning no caching, no cost tracking, no auto-model-selection, and no retry logic from the provider. [FACT] (`plan_creator.py:158-162`, `plan_reviewer.py:125-129`) (see Module Index: kosmos/orchestration/ for full details) (see Module Index: kosmos/core/providers/base.py for what is bypassed)
...
47. **Most retry fixes are try/except wrappers** — 8 of 11 error-type fixes simply wrap entire code in try/except, producing `results = {'error': ..., 'status': 'failed'}` rather than fixing the underlying issue. Callers must check `result.return_value` for `{'status': 'failed'}` in addition to `result.success`. [FACT] (`executor.py:869-1008`) (see Error Handling: Code Execution Retry for full retry strategy) (see Module Index: kosmos/execution/executor.py for RetryStrategy API)
...
53. **Auto-fix can insert an import the validator rejects** — RetryStrategy's `COMMON_IMPORTS` includes `'os': 'import os'`, but `os` is on the `DANGEROUS_MODULES` list. [FACT] (`code_validator.py:36`, `executor.py:686`) (see Module Index: kosmos/safety/code_validator.py and Module Index: kosmos/execution/executor.py for the conflicting validation and retry policies)
...
75. **`CONVERGED` can transition back to `GENERATING_HYPOTHESES`** — "Converged" is not truly terminal. Callers that assume convergence is final may be surprised. [FACT] (`workflow.py:212-213`) (see Module Index: kosmos/core/workflow.py for the full transition table)
